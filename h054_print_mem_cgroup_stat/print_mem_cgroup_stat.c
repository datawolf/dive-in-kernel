...
//	memcg = mem_cgroup_from_css(task_subsys_state(task, mem_cgroup_subsys_id));
	...
	for_each_online_cpu(cpu) {
		...
		val = per_cpu(memcg->stat_cpu->nr_page_events, cpu);
		printk(KERN_ALERT "cpu %d, nr_page_events = %ld", cpu, val);
		for (i = 0; i < MEM_CGROUP_NTARGETS; i++) {
			val = per_cpu(memcg->stat_cpu->targets[i], cpu);
			printk(KERN_ALERT "\t targets[%d] =  %ld", i, val);
		}
	}
	return 0;
}
